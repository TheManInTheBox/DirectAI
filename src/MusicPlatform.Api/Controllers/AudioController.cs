...
public class AudioController : ControllerBase
{
    ...
    public async Task<ActionResult<AudioFile>> UploadAudio(IFormFile file)
    {
        ...
        /// <summary>
        /// Get AI insights (Flamingo semantic analysis) for an audio file by reading
        /// the Bark training dataset JSON generated by the analysis worker.
        /// </summary>
        /// <param name="id">Audio file ID</param>
        /// <returns>Semantic insights (genre, mood, instruments, description, quality)</returns>
        [HttpGet("{id}/insights")]
        [ProducesResponseType(typeof(AudioInsightsDto), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<AudioInsightsDto>> GetAudioInsights(Guid id)
        {
            // Bark dataset is uploaded under: audio-files/{id}/bark_training/bark_training_dataset_{id}.json
            try
            {
                ...
                // Build expected blob path
                var datasetBlobName = $"{id}/bark_training/bark_training_dataset_{id}.json";
                var blobClient = containerClient.GetBlobClient(datasetBlobName);

                // If exact name not found, try to locate any dataset json in bark_training folder
                if (!(await blobClient.ExistsAsync()).Value)
                {
                    await foreach (var item in containerClient.GetBlobsAsync(prefix: $"{id}/bark_training/"))
                    {
                ...
                if (!exists.Value)
                {
                    return NotFound(new { message = "Insights not available yet. Try again after analysis completes." });
                }
                ...
                // Parse minimal fields dynamically to avoid tight coupling
                using var doc = System.Text.Json.JsonDocument.Parse(json);
                var root = doc.RootElement;

                // main_track.semantic_attributes
                ...
                if (root.TryGetProperty("main_track", out var mainTrack))
                {
                    if (mainTrack.TryGetProperty("text_description", out var descEl) && descEl.ValueKind == System.Text.Json.JsonValueKind.String)
                    {
                        description = descEl.GetString();
                    }
                    if (mainTrack.TryGetProperty("semantic_attributes", out var semEl))
                    {
                        if (semEl.TryGetProperty("genre", out var genEl) && genEl.ValueKind == System.Text.Json.JsonValueKind.String)
                        {
                            genre = genEl.GetString();
                        }
                        if (semEl.TryGetProperty("mood", out var moodEl) && moodEl.ValueKind == System.Text.Json.JsonValueKind.String)
                        {
                            mood = moodEl.GetString();
                        }
                        if (semEl.TryGetProperty("audio_quality", out var qualEl) && qualEl.ValueKind == System.Text.Json.JsonValueKind.String)
                        {
                            quality = qualEl.GetString();
                        }
                        if (semEl.TryGetProperty("instruments", out var instEl) && instEl.ValueKind == System.Text.Json.JsonValueKind.Array)
                        {
                            foreach (var i in instEl.EnumerateArray())
                            {
                                if (i.ValueKind == System.Text.Json.JsonValueKind.String)
                                {
                                    instruments.Add(i.GetString()!);
                                }
                            }
                        }
                    }
            ...
            {
                _logger.LogError(ex, "Error retrieving audio insights for {AudioFileId}", id);
                return StatusCode(500, new { message = "Failed to retrieve audio insights", error = ex.Message });
            }
        }
        ...
        try
        {
            ...
            if (existingResults != null)
            {
                ...
                existingResults = existingResults with
                {
                    ...
                    FlamingoInsightsJson = results.FlamingoInsightsJson,
...
/// <summary>
/// DTO for AI insights (Flamingo semantic analysis) returned to clients.
/// </summary>
...
public record AnalysisResultsRequest(
    ...
    string? FlamingoInsightsJson = null,
